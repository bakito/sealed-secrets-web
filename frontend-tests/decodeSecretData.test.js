...
beforeAll(() => {
  const yamlSrc = readFileSync(path.join(__dirname, '../static/scripts/yaml.min.js'), 'utf8')
  const windowStub = {}
  new Function('window', 'module', 'exports', 'require', yamlSrc)(windowStub, {}, {}, require) // eslint-disable-line no-new-func
  YAML = windowStub.YAML

  const decodeSrc = readFileSync(path.join(__dirname, '../static/scripts/decode.js'), 'utf8')
  decodeSecretData = new Function('YAML', 'atob', decodeSrc + '; return decodeSecretData;')(YAML, atob) // eslint-disable-line no-new-func
})
